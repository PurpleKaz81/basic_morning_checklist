...
def generate_html(data):
    ...
    for item in data["items"]:
        ...
        wake_time_section = ""
        if item["id"] == 1:
            wake_time_display = "none" if item["completed_today"] else "block"
            late_display = "none"
            late_text = ""
            
            if item.get("actual_wake_time") and item.get("minutes_late") is not None:
                minutes = item["minutes_late"]
                if minutes == 0:
                    late_text = "✅ On time!"
                elif minutes < 60:
                    late_text = f"⏰ {minutes} minutes late"
                else:
                    hours = minutes // 60
                    mins = minutes % 60
                    late_text = f"⏰ {hours} hour{'s' if hours > 1 else ''}" + (f" {mins} min" if mins > 0 else "") + " late"
                late_display = "block"
            
            wake_time_section = f"""
                <div id="wake-time-input" style="display: {wake_time_display}; margin-top: 10px; margin-left: 30px;">
                    <label for="wake-time">At what time did you wake up?</label>
                    <input type="time" id="wake-time" value="{item.get('actual_wake_time', '')}">
                    <button onclick="submitWakeTime()">Submit</button>
                </div>
                <div id="late-result" style="display: {late_display}; margin-top: 10px; margin-left: 30px; font-weight: bold; color: #e74c3c;">
                    {late_text}
                </div>
            """
...
<body>
    ...
    <script>
        function toggleItem(id) {{
            // Checkbox clicked - will reload page to show updated status
        ...
        function submitWakeTime() {{
            ...
            if (!wakeTime) {{
                alert('Please enter a wake time');
                return;
            }}
    ...
    for item in data["items"]:
        ...
        if item["id"] == 1 and item.get("actual_wake_time"):
            ...
            if minutes == 0:
                ...
                hours = minutes // 60
                mins = minutes % 60
                print(f"   ⏰ {hours} hour{'s' if hours > 1 else ''}" + (f" {mins} min" if mins > 0 else "") + " late")
